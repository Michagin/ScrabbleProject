module Dictionary
      ...
      let internal empty (s:string) = Node( Map.empty |> Map.toSeq |> Seq.append (seq {for c in s.ToCharArray() do yield (c,(Leaf,false))}) |> Map.ofSeq)

      let internal alphabet (dict:Dictionary) = 
       match dict with 
      ...
      let rec internal insert (s:string) (dict:Dictionary) = 
       match dict with
      ...
      let rec internal lookup (s:string) (dict:Dictionary) =
       match dict with
       | Leaf -> false
